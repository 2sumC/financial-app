  ...
  return (
    <div className="relative h-[400px] max-w-[400px] overflow-hidden rounded-2xl">
      ...
      >
        {slides.map((slide, index) => (
          ...
          >
            ...
            />
            <div className="absolute top-44 left-0 flex h-96 w-full flex-col gap-2 bg-gray-800/[0.6] p-4 text-white lg:gap-4">
              <p>{slide.text}</p>
      ...
      <img
        ...
        className="absolute right-16 bottom-5 h-7 w-7 rounded-full border p-1 hover:cursor-pointer"
        src={ArrowBack}
        alt="Previous slide button"
      />
      <img
        onClick={nextSlide}
        className="absolute right-5 bottom-5 h-7 w-7 rounded-full border p-1 hover:cursor-pointer"
        src={ArrowForward}
